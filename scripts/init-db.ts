...
const sampleTestimonials: TestimonialSeed[] = [
  {
    ...
    content:
      "Đội ngũ vô cùng nhiệt tình và chuyên nghiệp. Ảnh chụp rất tự nhiên, chúng mình cực kì ưng ý với bộ ảnh cưới này. Cảm ơn Harmony Studio rất nhiều!",
    rating: 5,
    avatar: null,
  ...
  },
  {
    ...
    content:
      "Dịch vụ quá tuyệt vời! Cả ekip đã tạo cho hai đứa một không gian rất thoải mái trong suốt buổi chụp. Mình đặc biệt thích phong cách blend màu phim của studio.",
    rating: 5,
    avatar: null,
  ...
  },
  {
    couple_name: "Hải & Yến",
    content:
      "Vợ chồng mình chọn Harmony sau khi xem qua rất nhiều portfolio của các bên. Và quả thực là một quyết định đúng đắn. Hình ảnh đẹp ngoài mong đợi và video cưới cực kì xịn xò.",
    rating: 5,
    avatar: null,
  ...
  },
  {
    ...
    content:
      "Concept chụp của team rất đa dạng và hợp gu. Bạn makeup cũng làm tóc và trang điểm cực kì tự nhiên, tôn lên những đường nét đẹp nhất. Highly recommend nhé!",
    rating: 5,
    avatar: null,
  ...
  },
  {
    ...
    content:
      "Tụi mình chụp ở phim trường Vũ Garden, nắng hôm đó rất gắt nhưng ekip hỗ trợ nhiệt tình từ lúc sáng sớm tới chiều tối. Kết quả là những bức hình lưu giữ kỉ niệm không thể tuyệt hơn.",
    rating: 5,
    avatar: null,
...
function inferLocationType(folderName: string): LocationType {
  if (folderName.includes("đường phố") || folderName.includes("outdoor"))
    return "outdoor";
  return "studio";
}
...
function runMigrations(db: Database.Database) {
  ...
  const portfolioColumns = db.pragma("table_info(portfolios)") as {
    name: string;
  }[];
  const hasOrientation = portfolioColumns.some(
    (column) => column.name === "orientation",
  );
...
async function seedPortfoliosFromWedding(db: Database.Database) {
  if (!fs.existsSync(WEDDING_DIR)) {
    console.log(
      "· wedding/ directory not found — skipped portfolio image seeding",
    );
    return;
  }
  ...
  for (const dir of dirs) {
    ...
    console.log(
      `Processing portfolio seed folder: ${folderName} (${sourceImages.length} images)`,
    );
  ...
  console.log(
    created > 0
      ? `✓ Seeded ${created} portfolio(s)`
      : "· portfolios already seeded — skipped",
  );
}
...
function getPortfolioRows(db: Database.Database): PortfolioSeedRow[] {
  return db
    .prepare(
      "SELECT slug, title, images FROM portfolios ORDER BY sort_order ASC, created_at DESC",
    )
    .all() as PortfolioSeedRow[];
}
...
function seedStudios(db: Database.Database) {
  ...
  const studioPortfolio = portfolios.find((portfolio) =>
    portfolio.slug.includes("studio"),
  );
  ...
  const locations: Array<{
    ...
    {
      name: "Phim Trường An Garden",
      slug: "phim-truong-an-garden",
      type: "studio",
      address: "Quận 9, TP. HCM",
    },
    {
      name: "Phim Trường Sunny Garden",
      slug: "phim-truong-sunny-garden",
      type: "studio",
      address: "Quận 9, TP. HCM",
    },
    {
      name: "Phim Trường Vũ Garden",
      slug: "phim-truong-vu-garden",
      type: "studio",
      address: "Lái Thiêu, Bình Dương",
    },
    {
      name: "Đường Phố Hồ Chí Minh",
      slug: "duong-pho-ho-chi-minh",
      type: "outdoor",
      address: "Trung tâm Quận 1 & Quận 3, TP. HCM",
    },
  ];
  ...
  insertStudio.run({
    ...
    city: "Đồng Nai",
    description:
      "Studio chính của Harmony tại Đồng Nai, với không gian rộng rãi và nhiều góc chụp concept đa dạng.",
    highlights: JSON.stringify([
      "Trang thiết bị hiện đại",
      "Makeup chuyên nghiệp",
      "Phòng thay đồ riêng tư",
      "Bối cảnh đa dạng",
    ]),
  ...
  locations.forEach((location, index) => {
    ...
    insertStudio.run({
      ...
      map_embed_url: null,
      best_time:
        location.type === "outdoor" ? "Sáng sớm hoặc chiều tà" : "Cả ngày",
      is_active: 1,
      sort_order: index + 2,
    });
  });
...
function seedHeroAndGallery(db: Database.Database) {
  ...
  if (tableCount(db, "gallery_items") === 0) {
    ...
    for (const portfolio of portfolios) {
      for (const [index, src] of parseImages(portfolio.images)
        .slice(0, 4)
        .entries()) {
        insertGallery.run({
          src,
