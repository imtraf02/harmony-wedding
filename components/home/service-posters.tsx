  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[1500px] px-5 md:px-10 lg:px-16">
        {/* Clean Poster Cards Grid — 3 columns on desktop, 2 columns on mobile */}
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 max-w-6xl mx-auto">
          {POSTERS.map((poster) => (
            <Link 
              ...
              key={poster.src} 
              className="poster-item group relative block overflow-hidden rounded-2xl sm:rounded-3xl bg-neutral-100 shadow-md sm:shadow-xl border border-black/5 aspect-[9/16] w-full max-w-[360px] mx-auto transition-transform duration-500 hover:translate-y-[-6px] cursor-pointer"
            >
