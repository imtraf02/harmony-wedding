...
export function getHeroSlides() {
  const db = getDb();
  const items = db.prepare('SELECT * FROM hero_slides WHERE is_active = 1 ORDER BY sort_order ASC').all() as any[];
...
export function getAllHeroSlides() {
  const db = getDb();
  const items = db.prepare('SELECT * FROM hero_slides ORDER BY sort_order ASC').all() as any[];
  return items.map(item => ({
    ...item,
    is_active: item.is_active === 1
  })) as HeroSlide[];
...
export function getHeroSlideById(id: number) {
  const db = getDb();
  const item = db.prepare('SELECT * FROM hero_slides WHERE id = ?').get(id) as any;
  if (!item) return null;
  return {
    ...item,
    is_active: item.is_active === 1
...
export function updateHeroSlide(id: number, data: Partial<Omit<HeroSlide, 'id' | 'created_at'>>) {
  const db = getDb();
