...
export function getPublishedPosts(): Post[] {
  ...
  ).all() as Record<string, unknown>[]).map(parsePost);
}
...
export function updatePost(id: number, data: Partial<Omit<Post, 'id' | 'created_at'>>): void {
  const db = getDb();
  const sets: string[] = [];
  ...
  for (const [key, val] of Object.entries(data)) {
    sets.push(`${key} = @${key}`);
    params[key] = key === 'tags' ? JSON.stringify(val) : key === 'is_published' ? (val ? 1 : 0) : val;
  }
