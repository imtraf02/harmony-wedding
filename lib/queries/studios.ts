...
export function getStudioBySlug(slug: string): Studio | null {
  const db = getDb();
  const row = db.prepare("SELECT * FROM studios WHERE slug = ?").get(slug) as
...
export function getStudioById(id: number): Studio | null {
  const db = getDb();
  const row = db.prepare("SELECT * FROM studios WHERE id = ?").get(id) as
    | Record<string, unknown>
    | undefined;
  return row ? parseStudio(row) : null;
}
...
export function getAllStudiosAdmin(): Studio[] {
  const db = getDb();
  return (
    db.prepare("SELECT * FROM studios ORDER BY sort_order ASC").all() as Record<
      string,
      unknown
    >[]
  ).map(parseStudio);
