...
export function getAllTestimonials() {
  ...
  return (
    db
      .prepare("SELECT * FROM testimonials ORDER BY sort_order ASC, id DESC")
...
export function getActiveTestimonials(limit = 10) {
  ...
  return (
    db
      .prepare(
        "SELECT * FROM testimonials WHERE is_active = 1 ORDER BY sort_order ASC, id DESC LIMIT ?",
      )
...
export function updateTestimonial(
  ...
  db.prepare(`UPDATE testimonials SET ${sets.join(", ")} WHERE id = @id`).run(
    params,
  );
}
