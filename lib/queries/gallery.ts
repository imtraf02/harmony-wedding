...
export function getAllGalleryItems(): GalleryItem[] {
  const db = getDb();
  return (
    db.prepare('SELECT * FROM gallery_items ORDER BY sort_order ASC, id ASC').all() as Record<
      string,
      unknown
    >[]
  ).map(parseGalleryItem);
...
export function updateGalleryItem(
  ...
  db.prepare(`UPDATE gallery_items SET ${sets.join(', ')} WHERE id = @id`).run(params);
}
...
export function reorderGalleryItems(orderedIds: number[]): void {
  const db   = getDb();
  const stmt = db.prepare('UPDATE gallery_items SET sort_order = @order WHERE id = @id');
